      ...
      <ul class="card-opciones list-unstyled mb-0 flex-grow-1 text-start min-w-0">
        ...
        >
          <div class="card-opcion__info min-w-0">
            ...
            <p v-if="!hasTemas" class="card-opcion__price mb-0">{{ opcion.price }}</p>
          </div>
          <button
            v-if="!agotado && hasTemas"
            type="button"
            class="btn btn-whatsapp wa-pill-btn card-opcion__wa"
            :aria-label="`Obras disponibles de ${opcion.nombre} para ${title}`"
            @click="openTrackModal(opcion)"
          >
            <svg
              class="wa-pill-icon"
            ...
            <span class="wa-pill-label">Obras disponibles</span>
          </button>
          <a
            v-else-if="!agotado"
            class="btn btn-whatsapp wa-pill-btn card-opcion__wa"
...
function openTrackModal(opcion) {
  modalFormato.value = opcion
  modalOpen.value = true
}

function closeTrackModal() {
  modalOpen.value = false
  modalFormato.value = null
}
